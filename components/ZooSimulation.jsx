  ...
  function visitAnimal(name) {
    ...
    if ("speechSynthesis" in window) {
      window.speechSynthesis.speak(
        new SpeechSynthesisUtterance(`Visiting the ${name}. ${animalObj ? animalObj.fact : ""}`),
      );
    }
  }
          ...
          <div key={a.name} style={{ textAlign: "center", transition: "transform 0.3s" }}>
            <img
              ...
              style={{
                ...
                height: "60px",
                objectFit: "contain",
                filter: animal === a.name ? "brightness(1.2)" : "none",
                transition: "filter 0.3s",
              }}
            />
            <button onClick={() => visitAnimal(a.name)} aria-label={`Visit ${a.name}`}>
              {a.name}
            </button>
