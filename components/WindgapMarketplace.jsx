...
const WindgapMarketplace = () => {
  ...
  useSpeechRecognition({
    commands: [
      ...
      },
      {
        ...
        action: () => { setShowChat(false); alert('Closed chat (voice command)'); },
      },
      {
        phrase: /^send (.+)$/,
      ...
      },
      {
        ...
        action: (phrase) => {
          ...
          if (char) {
            const itemMap = {
              a: 'apple', b: 'broccoli', c: 'carrot', d: 'daikon', e: 'eggplant', f: 'fennel', g: 'garlic', h: 'horseradish', i: 'iceberg lettuce', j: 'jalapeno', k: 'kale', l: 'leek', m: 'mushroom', n: 'napa cabbage', o: 'onion', p: 'potato', q: 'quince', r: 'radish', s: 'spinach', t: 'tomato', u: 'ugli fruit', v: 'vanilla bean', w: 'watermelon', x: 'xigua (Chinese melon)', y: 'yam', z: 'zucchini', '.': 'salt', ',': 'pepper', '+': 'ingredient', '-': 'ingredient (removed)'
            };
  ...
  const handleMiniGameComplete = () => {
    ...
    setLeaderboard(prev => [{ name: 'You', score: (prev[0]?.score || 0) + reward }, ...prev.slice(1)]);
  ...
  const Modal = ({ show, onClose, title, children, ariaLabel }) => (
    show ? (
      <div role="dialog" aria-modal="true" aria-label={ariaLabel || title} className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full">
          {title && <h2 className="text-xl font-bold mb-2">{title}</h2>}
          <div>{children}</div>
          {onClose && <button className="mt-4 bg-blue-500 text-white px-4 py-2 rounded" onClick={onClose} aria-label={`Close ${ariaLabel || title}`}>Close</button>}
        </div>
      </div>
    ) : null
  );
    ...
    <main className="marketplace-container relative" aria-label="Windgap Marketplace" role="main">
      <StickyNavbar>
        ...
        <DropdownMenu label="Menu">
          <a href="/buy" className="block px-4 py-2 hover:bg-blue-100" role="menuitem">Buy</a>
          <a href="/sell" className="block px-4 py-2 hover:bg-blue-100" role="menuitem">Sell</a>
          <a href="/exchange" className="block px-4 py-2 hover:bg-blue-100" role="menuitem">Exchange</a>
        </DropdownMenu>
      </StickyNavbar>
      <Modal show={showOnboarding} onClose={() => setShowOnboarding(false)} title="Welcome to Windgap Marketplace!" ariaLabel="Onboarding">
        <p className="mb-4">Explore the campus, interact with buildings, play mini-games, and join the ClubHouse for social events.</p>
        <button className="bg-blue-500 text-white px-4 py-2 rounded" onClick={() => setShowOnboarding(false)} aria-label="Close onboarding">Get Started</button>
      </Modal>
      ...
      <div className="w-full h-[60vh] md:h-[70vh] lg:h-[80vh] relative bg-gradient-to-b from-blue-300 to-indigo-500">
        ...
        <div className="absolute left-8 top-8 z-30">
          <Tooltip text="Animated mascot">
            <LottieAnimation src="/assets/lottie/winnie.json" loop={true} style={{ width: 80, height: 80 }} />
          </Tooltip>
        </div>
        ...
        <Tooltip text="Relax and recharge">
          <button className="absolute left-1/2 bottom-24 transform -translate-x-1/2 bg-teal-500 text-white px-6 py-3 rounded-full shadow-lg text-lg font-semibold" aria-label="Enter Calm Space" onClick={() => alert('Welcome to Calm Space!')}>Enter Calm Space</button>
        </Tooltip>
        {/* Avatars and leaderboard overlays */}
        <div className="absolute top-4 right-4 bg-white bg-opacity-80 rounded-lg p-2 shadow-md" aria-label="Leaderboard">
          <h3 className="font-bold text-sm mb-1">Leaderboard</h3>
          <ul>
            {leaderboard.map((entry, i) => (
              <li key={i} className="text-xs">{entry.name}: {entry.score}</li>
            ))}
          </ul>
        </div>
        <div className="absolute bottom-4 left-4 bg-white bg-opacity-80 rounded-lg p-2 shadow-md" aria-label="Avatars">
        ...
        <button className="absolute bottom-4 right-4 bg-green-500 text-white px-3 py-2 rounded shadow-md" onClick={handleMiniGameClick} aria-label="Play Mini-Game">Play Mini-Game</button>
        <button className="absolute bottom-16 right-4 bg-orange-500 text-white px-3 py-2 rounded shadow-md" onClick={handleClubHouseClick} aria-label="Open ClubHouse">ClubHouse</button>
        {/* Mini-game modal */}
        <Modal show={showMiniGame} onClose={() => setShowMiniGame(false)} title="Mini-Game" ariaLabel="Mini-Game">
          <p className="mb-4">Complete the challenge to earn tokens!</p>
          <button className="bg-green-500 text-white px-4 py-2 rounded mb-2" onClick={handleMiniGameComplete}>Finish Game</button>
        </Modal>
        {/* ClubHouse chat modal */}
        <Modal show={showChat} onClose={handleCloseChat} title="ClubHouse Chat" ariaLabel="ClubHouse Chat">
          ...
          <input type="text" className="border rounded px-2 py-1 mb-2" placeholder="Type a message..." />
      ...
      <form className="mt-8 p-4 bg-gray-100 rounded-lg shadow" aria-label="Feedback Form" onSubmit={e => { e.preventDefault(); alert('Feedback submitted!'); }}>
        <label htmlFor="feedback" className="block font-bold mb-2">Feedback:</label>
        <textarea id="feedback" name="feedback" rows={3} className="w-full p-2 border rounded mb-2" aria-required="true" />
