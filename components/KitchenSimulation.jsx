...
function KitchenSimulation() {
  const [ingredients, setIngredients] = useState(
    SimulationManager.getProgress("kitchen").ingredients || [],
  );
  ...
  return (
    ...
    >
          ...
          <div key={ing.name} style={{ textAlign: "center", transition: "transform 0.3s" }}>
            <img
              ...
              style={{
                ...
                height: "60px",
                objectFit: "contain",
                filter: ingredients.includes(ing.name) ? "brightness(1.2)" : "none",
                transition: "filter 0.3s",
              }}
            />
            <button onClick={() => addIngredient(ing)} aria-label={`Add ${ing.name}`}>
              Add {ing.name}
            </button>
          </div>
        ))}
      ...
      <div>
        ...
        <ul>
          {ingredients.map((item, idx) => (
            <li key={idx} style={{ transition: "background 0.3s", background: "#ffe" }}>
              {item}
            </li>
          ))}
        </ul>
      ...
      <div>
        ...
        >
          Step {step + 1}: {recipeSteps[step]}
        </p>
        <button onClick={nextStep} disabled={step >= recipeSteps.length - 1}>
          Next Step
        </button>
      </div>
    </section>
