...
export default function CurriculumEditor({ topic }: { topic: CurriculumTopic }) {
  ...
  const handleSave = () => {
    firestore.collection("curriculum").doc(topic.topicId).set(metadata);
  };
  ...
  return (
    <div className="bg-white p-4 rounded shadow">
      ...
      <select multiple value={metadata.acsfSkills} onChange={(e) => setMetadata({ ...metadata, acsfSkills: Array.from(e.target.selectedOptions).map(o => o.value) })}>
