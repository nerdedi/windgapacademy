...
function ClubhouseSimulation() {
  const [activity, setActivity] = useState(SimulationManager.getProgress('clubhouse').activity || 'welcome');
  ...
  return (
    <section aria-label="Clubhouse Simulation" tabIndex={0} style={{ border: '2px solid #eee', padding: '1rem', borderRadius: '8px', background: '#f0fff8', transition: 'background 0.5s' }}>
      <h2>Clubhouse Simulation</h2>
      <p>{message}</p>
      <div style={{ display: 'flex', gap: '2rem', marginBottom: '1rem' }}>
        {activities.map(act => (
          <div key={act.name} style={{ textAlign: 'center', transition: 'transform 0.3s' }}>
            <img src={act.img} alt={act.name} style={{ width: '60px', height: '60px', objectFit: 'contain', filter: activity === act.name ? 'brightness(1.2)' : 'none', transition: 'filter 0.3s' }} />
            <button onClick={() => startActivity(act.name)} aria-label={`Start ${act.name}`}>{act.name}</button>
      ...
      <div>
        ...
        <p style={{ fontWeight: 'bold', fontSize: '1.2em', transition: 'color 0.3s', color: '#00796b' }}>{activity}</p>
      </div>
    </section>
  );
}
