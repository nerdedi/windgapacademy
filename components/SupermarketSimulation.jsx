...
function SupermarketSimulation() {
  ...
  const [location, setLocation] = useState(
    SimulationManager.getProgress("supermarket").location || "entrance",
  );
  ...
  function checkout() {
    ...
    if ("speechSynthesis" in window) {
      window.speechSynthesis.speak(
        new SpeechSynthesisUtterance("Checking out. Thank you for shopping!"),
      );
    }
    setCart([]);
  ...
  return (
    <section
      aria-label="Supermarket Simulation"
      tabIndex={0}
      style={{
    ...
    >
      ...
      <div
        ...
        aria-label="Drop items here to add to cart"
      >
        <h3>Your Cart (Drag items here)</h3>
        <ul>
          {cart.map((item, idx) => (
            <li key={idx} style={{ transition: "background 0.3s" }}>
              {item}
            </li>
          ))}
        </ul>
