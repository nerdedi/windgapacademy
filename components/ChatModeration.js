...
export function moderateChat(message, userId = null) {
  for (var i = 0; i < bannedWords.length; i++) {
    ...
    if (message.toLowerCase().indexOf(word) !== -1) {
      ...
      if (window.logEducatorAction) window.logEducatorAction({ type: 'chatModeration', message: message, word: word });
      // Show privacy notice
      ...
      if (userId) {
        saveChatLog(userId, [{ message: message, banned: true, reason: 'Profanity or bullying detected', word: word, timestamp: Date.now() }]).catch(function(err) {
