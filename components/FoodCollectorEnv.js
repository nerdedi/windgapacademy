...
export class FoodCollectorEnv extends EnvironmentBase {
  ...
  render() {
    ...
    window.fcReset = () => { this.initGame(); this.render(); };
  }
  ...
  checkFood() {
    ...
    if (idx !== -1) {
      ...
      if (food.type === 'healthy') {
        ...
        this.container.querySelector('.env-feedback').textContent = `Great! You collected a healthy food: ${food.name}`;
      } else {
        this.agent.score -= 1;
        this.container.querySelector('.env-feedback').textContent = `Oops! ${food.name} is not a healthy choice.`;
      }
