...
class CurriculumAI {
  ...
  async getLearningPaths(userId) {
    try {
      const pathsQuery = query(collection(this.db, "learningPaths"), where("userId", "==", userId));
  ...
  async updateLearningPath(pathId, updates) {
    try {
      // Get current path
      let path = this.cache.learningPaths.get(pathId);
  ...
  async createPersonalizedCurriculum(nodes, skillGaps, learnerProfile) {
    ...
    const scoredNodes = nodes.map((node) => {
      ...
      const styleMatch = node.learningStyles.filter((style) =>
        learnerProfile.preferredLearningStyles.includes(style),
      ).length;

      relevanceScore +=
        (styleMatch / node.learningStyles.length) * this.config.personalizationWeight;

      // Adjust for interests
      const interestMatch = node.metadata.topics
        ? node.metadata.topics.filter((topic) => learnerProfile.interests.includes(topic)).length
        : 0;

      if (node.metadata.topics && node.metadata.topics.length > 0) {
        relevanceScore +=
          (interestMatch / node.metadata.topics.length) * this.config.interestWeight;
      }
    ...
    for (const [_skillId, nodeIds] of skillNodeMap.entries()) {
      if (nodeIds.length < 2) continue;

      const skillNodes = nodeIds
      ...
      for (let i = 0; i < skillNodes.length - 1; i++) {
        ...
        const edgeExists = curriculumEdges.some(
  ...
  async getRecommendations(userId, pathId, limit = 3) {
    try {
      ...
      const recommendations = this.rankAndAdjustRecommendations(nextNodes, learnerProfile);
  ...
  getNextNodes(path, limit) {
    ...
    if (!path.currentNode) {
      ...
      return path.nodes.filter((node) => rootNodeIds.includes(node.id)).slice(0, limit);
    }

    // Find nodes connected to current node
    const currentNodeId =
      typeof path.currentNode === "string" ? path.currentNode : path.currentNode.id;

    const outgoingEdges = path.edges.filter(
      (edge) =>
    ...
    return path.nodes.filter((node) => nextNodeIds.includes(node.id)).slice(0, limit);
  }
  ...
  rankAndAdjustRecommendations(nodes, learnerProfile) {
    ...
    const recommendations = nodes.map((node) => {
      ...
      if (node.learningStyles.length > 0) {
        relevance *=
          1 + (styleMatch / node.learningStyles.length) * this.config.personalizationWeight;
      }
  ...
  async trackProgress(userId, nodeId, completed = true) {
    try {
      ...
      const paths = await this.getLearningPaths(userId);
      const relevantPaths = paths.filter((path) => path.nodes.some((node) => node.id === nodeId));
      ...
      if (completed) {
        ...
        this.analytics.pathCompletionRate =
          paths.length > 0 ? completedPaths.length / paths.length : 0;
      }
  ...
  async generateKnowledgeMap(skillIds) {
    try {
      ...
      const relationships = this.skillTaxonomy.relationships.filter(
        (rel) => skillIds.includes(rel.source) && skillIds.includes(rel.target),
      );
...
export { CurriculumNode, LearnerProfile, LearningModel, LearningPath };
