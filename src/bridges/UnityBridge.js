...
class PerformanceMonitor {
  ...
  collectMetrics(metricTypes) {
    ...
    if (metricTypes.includes("memory")) {
      metrics.memory = performance.memory ? performance.memory.usedJSHeapSize / 1048576 : 0;
    }
...
class AnimationSynchronizer {
  ...
  interpolate(timestamp) {
    ...
    const t =
      (timestamp - this.interpolationBuffer[0].timestamp) /
      (this.interpolationBuffer[1].timestamp - this.interpolationBuffer[0].timestamp);

    return this.cubicInterpolation(this.interpolationBuffer[0], this.interpolationBuffer[1], t);
  }

  cubicInterpolation(a, b, t) {
    const t2 = t * t;
    const _t3 = t2 * t; // Unused variable prefixed with _ to satisfy ESLint

    // Cubic Bezier interpolation
    return {
      frame: a.frame + (b.frame - a.frame) * t,
      blendWeights: a.blendWeights.map((w, i) => w + (b.blendWeights[i] - w) * t),
    };
  }
}
...
class UnityBridge {
        ...
        script.onload = () => {
          if (typeof window.createUnityInstance === "undefined") {
            const error = new Error(
              "Unity loader script loaded but createUnityInstance is undefined",
            );
  ...
  setupCommunicationChannel() {
    ...
    window.SendToUnity = (gameObject, method, parameter) => {
      if (this.unityInstance) {
        ...
        this.log("Message queued for Unity (not yet initialized):", {
          gameObject,
          method,
          parameter,
        });
  ...
  processMessageQueue() {
    ...
    while (this.messageQueue.length > 0) {
      ...
      this.unityInstance.SendMessage(message.gameObject, message.method, message.parameter);
...
export { AnimationSynchronizer, PerformanceMonitor };
...
export const sendToUnity = (gameObject, method, parameter) => {
  ...
  console.warn(
    "[UnityBridge] Using deprecated sendToUnity method. Please use UnityBridge instance instead.",
  );
...
export const registerUnityMessageHandler = (type, callback) => {
  // This is a static method for backwards compatibility
  console.warn(
    "[UnityBridge] Using deprecated registerUnityMessageHandler method. Please use UnityBridge instance instead.",
  );
