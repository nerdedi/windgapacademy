...
export const MicroInteractionsProvider = ({ children, initialTheme = 'default' }) => {
  ...
  useEffect(() => {
    ...
    const updateFPS = () => {
      ...
      if (now - frameCounter.current.lastCheck >= 1000) {
        const fps = Math.round(frameCounter.current.count * 1000 / (now - frameCounter.current.lastCheck));
  ...
  const applyPreset = useCallback((presetName, customProps = {}) => {
...
export const useStaggeredAnimation = (itemCount, staggerDelay = 0.05, delayStart = 0) => {
  ...
  return {
    ...
    itemDelays: Array(itemCount).fill(0).map((_, i) => i * finalStaggerDelay + delayStart)
  };
};
...
export const useAnimatedValue = (initialValue, config = {}) => {
  ...
  const setValue = useCallback((newValue, animate = true) => {
    if (animate) {
