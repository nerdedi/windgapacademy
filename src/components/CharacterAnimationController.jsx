...
const CharacterAnimationController = ({ 
  ...
  const handleUnityMessage = useCallback((actionType, data) => {
    ...
    if (actionType === 'ANIMATION_STARTED') {
      // Add to log
      setAnimationLog(prev => [...prev, `Animation started: ${data.animationName} for ${data.characterName}`]);
      setCurrentAnimation(data.animationName);
    }
    
    if (actionType === 'ANIMATION_COMPLETED') {
      // Add to log
      setAnimationLog(prev => [...prev, `Animation completed: ${data.animationName} for ${data.characterName}`]);
      setCurrentAnimation('');
    }
  }, []);
  
  // Play animation
  const handlePlayAnimation = useCallback((animationName, duration = 0) => {
    ...
    // Add to log
    setAnimationLog(prev => [...prev, `Requested animation: ${animationName}${duration ? ` (${duration}s)` : ''}`]);
  ...
  // Play sequence
  const handlePlaySequence = useCallback((sequenceName) => {
    if (!isUnityLoaded || currentAnimation) return;
