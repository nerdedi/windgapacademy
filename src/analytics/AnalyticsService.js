...
class AnalyticsService {
  ...
  trackEvent(eventName, eventData = {}) {
    ...
    const event = {
      ...
      engagementScore: this.calculateEventEngagementScore(
        eventName,
        eventData,
        timeSinceLastActivity,
      ),
  ...
  processEventForLearningPatterns(event) {
    ...
    if (
      !["lesson_complete", "quiz_complete", "exercise_complete", "video_complete"].includes(
        eventName,
      )
    ) {
      return;
    ...
    const recencyFactor = Math.exp(
      -((Date.now() - pattern.lastInteraction) / (7 * 24 * 60 * 60 * 1000)),
    ); // Decay over a week
  ...
  async sendToAggregateAnalytics(events) {
    try {
      ...
      const avgEngagement =
        events.reduce((sum, event) => sum + event.engagementScore, 0) / events.length;
  ...
  async handleUnload() {
    ...
    this.trackEvent("session_end", {
      sessionDuration:
        Date.now() - this.events.find((e) => e.eventName === "session_start")?.timestamp,
  ...
  getInsights() {
    ...
    const weaknesses = Object.entries(this.learningPatterns)
      .filter(([_key, data]) => data.mistakes.length > 0 || data.strength < 0.3)
      .map(([topic]) => topic);
    ...
    if (completions.length >= 2) {
      const avgTimeBetweenCompletions =
        completions
          .slice(1)
          .reduce((sum, event, i) => sum + (event.timestamp - completions[i].timestamp), 0) /
        (completions.length - 1);

      if (avgTimeBetweenCompletions < 300000) {
        // < 5 minutes
        learningPace = "fast";
      } else if (avgTimeBetweenCompletions > 1800000) {
        // > 30 minutes
        learningPace = "slow";
      }
    }
