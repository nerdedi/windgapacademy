...
const AnalyticsVisualizer = ({ userId, timeRange = 'month', className = '' }) => {
  ...
  const topicData = useMemo(() => {
    ...
    return Object.entries(insights.timeByTopic || {}).map(([topic, time]) => ({
      topic,
      time,
      strength: insights.strengths.find(s => s === topic) ? 'high' : 
                insights.weaknesses.find(w => w === topic) ? 'low' : 'medium'
    }));
  }, [insights]);
  ...
  const renderRadarChart = () => {
    ...
    svg.selectAll('.radar-point')
      ...
      .attr('r', 5)
      .attr('fill', d => d.strength === 'high' ? '#4CAF50' : d.strength === 'low' ? '#F44336' : '#FFC107')
      .attr('stroke', '#fff')
      .attr('stroke-width', 2);
  ...
  const renderTopicsChart = () => {
    ...
    svg.append('g')
      .attr('transform', `translate(0,${height})`)
      .call(d3.axisBottom(x).ticks(5).tickFormat(d => `${Math.round(d / (60 * 1000))} min`));
  ...
  const renderProgressChart = () => {
    if (!progressChartRef.current || !recommendations || !recommendations.recommendedLearningPath) return;
      ...
      <div className={`analytics-visualizer ${className}`}>
        <div className="text-center p-8 bg-gray-50 rounded-lg">
          ...
          <p className="text-gray-500 mt-2">
            Start learning to generate analytics insights.
          </p>
        </div>
      </div>
    );
              ...
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <h4 className="font-semibold text-blue-800">
                    {recommendations?.learningPace === 'fast' ? 'Fast-Paced Visual Learner' :
                     recommendations?.learningPace === 'slow' ? 'Methodical Deep Learner' :
            ...
            <div>
              ...
              <ul className="space-y-2">
                {insights.strengths.map((topic, idx) => (
                  <li key={`strength-${idx}`} className="flex items-center p-2 bg-green-50 rounded-md">
            ...
            <div>
              ...
              <ul className="space-y-2">
                {insights.weaknesses.map((topic, idx) => (
                  <li key={`weakness-${idx}`} className="flex items-center p-2 bg-red-50 rounded-md">
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(insights.preferredContentTypes || {}).map(([type, count], idx) => (
                <div key={`content-${idx}`} className="px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-sm">
              ...
              {recommendations?.recommendedTimePerSession && (
                <div className="p-4 bg-blue-50 rounded-lg">
                  ...
                  <p className="text-blue-600">
                    Based on your learning patterns, aim for {recommendations.recommendedTimePerSession} minute sessions for optimal engagement and retention.
                  </p>
                </div>
              )}
              ...
              {recommendations?.needsBreak && (
                <div className="p-4 bg-amber-50 rounded-lg">
                  ...
                  <p className="text-amber-600">
                    Your recent engagement metrics suggest you might benefit from taking a short break before continuing.
                  </p>
                </div>
              )}
                ...
                <div className="p-4 bg-green-50 rounded-lg">
                  ...
                  <ul className="mt-2 space-y-1">
                    {recommendations.topicsToReinforce.map((topic, idx) => (
                      <li key={`revisit-${idx}`} className="text-green-600">• {topic}</li>
                    ))}
                  </ul>
                </div>
              )}
                ...
                <div className="p-4 bg-purple-50 rounded-lg">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {recommendations.effectiveContentTypes.map((type, idx) => (
                      <div key={`effective-${idx}`} className="px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-sm">
                        {type}
                      </div>
                    ))}
                  </div>
