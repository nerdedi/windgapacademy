...
class ParticleSystem {
  ...
  createParticle(index) {
    ...
    // Set lifetime
    this.lifetimes[index] = this.options.lifetime.min + Math.random() * (this.options.lifetime.max - this.options.lifetime.min);

    // Set size
    this.sizes[index] = this.options.size.min + Math.random() * (this.options.size.max - this.options.size.min);
...
export const useRippleEffect = (options = {}) => {
  ...
  const addRipple = useCallback((screenX, screenY) => {
...
class WebGLEffects {
  ...
  handleResize() {
    ...
    // Update FXAA pass
    const fxaaPass = this.composer.passes.find(pass => pass.material?.uniforms?.resolution !== undefined);
    if (fxaaPass) {
      const pixelRatio = this.renderer.getPixelRatio();
      fxaaPass.material.uniforms['resolution'].value.x = 1 / (width * pixelRatio);
      fxaaPass.material.uniforms['resolution'].value.y = 1 / (height * pixelRatio);
...
export { ParticleSystem, WeatherEffects, WebGLEffects, RippleShader, GlowShader };
