// Global app-level store for simple shared state (used by some legacy modules)
...
export const useStore = create((set) => ({
  // Minimal shape expected by various UI/analytics modules
  userData: null,
  userPreferences: {
    theme: "light",
    notifications: true,
...
const trackingMiddleware = (config) => (set, get, api) =>
  config(
    (...args) => {
      // Log state changes
      const prevState = get();
...
export const createStore = ({
  ...
  // Build a middleware applier. This composes wrappers around a base config
  const applyMiddlewares = (baseConfig) => {
    let wrapped = baseConfig;

    // Add immer middleware for immutable updates with mutable syntax
    wrapped = immer(wrapped);

    // Add validation middleware
    if (Object.keys(validators).length > 0) {
      wrapped = createValidationMiddleware(validators)(wrapped);
    }

    // Add analytics tracking
    wrapped = createAnalyticsMiddleware(name)(wrapped);

    // Add history middleware for time-travel debugging
    if (enableHistory) {
      wrapped = historyMiddleware(wrapped);
    }

    // Add cross-tab synchronization
    if (enableSync) {
      wrapped = createSyncMiddleware(name)(wrapped);
    }

    // Add state migrations
    if (version > 1 && Object.keys(migrations).length > 0) {
      wrapped = createMigrationMiddleware(version, migrations)(wrapped);
    }

    // Add devtools for debugging
    if (enableDevTools) {
      wrapped = devtools(wrapped, { name });
    }
  ...
  const optimizedSelectors = Object.entries(selectors).reduce((acc, [selectorName, selectorFn]) => {
    // Create a wrapper function with a React-compliant name (starts with 'use')
    acc[selectorName] = function useSelectorHook() {
...
export const createCurriculumStore = () => {
  ...
  const computed = {
    totalCompletedCount: (state) => Object.values(state.progress).filter((p) => p.completed).length,
    ...
    courseProgress: (state) => (courseId) => {
      ...
      const completedModules = courseModules.filter(
        (m) => state.progress[m.id] && state.progress[m.id].completed,
      ).length;
    ...
    moduleProgress: (state) => (moduleId) => {
      ...
      const completedLessons = moduleLessons.filter(
        (l) => state.progress[l.id] && state.progress[l.id].completed,
      ).length;
    ...
    getRecommendedContent: (state) => {
      ...
      ].filter((content) => !state.progress[content.id] || !state.progress[content.id].completed);
...
export const createGameStore = () => {
  ...
  const computed = {
    ...
    sessionDuration: (state) => {
      ...
      const endTime = state.sessionData.endTime ? new Date(state.sessionData.endTime) : new Date();
  ...
  const selectors = {
    ...
    selectCourseWithUserProgress: (courseId) => {
      ...
      return {
        ...
        completionPercentage: curriculumStore.useStore((state) => state.courseProgress(courseId)),
      };
    },
  };
